...
管理控制台的 **Configured servers** 工作区会同时列出静态服务器定义和模板服务器定义。定义由来源与名称共同标识：静态目标使用
`mcpServers/<名称>`，模板服务器定义使用 `mcpTemplates/<名称>`。如果两个来源存在同名定义，两个条目仍可分别查看；模板定义对运行时匹配具有权威性，静态定义会标记为被遮蔽。

模板详情描述的是已保存的定义，而不是实时模板服务器实例。它显示规范化的传输与生命周期字段、脱敏后的密钥操作、Handlebars
语法状态、引用的请求上下文变量名称，以及有界的活动实例数量。请求上下文的值不会显示。

创建、编辑和重命名只使用结构化控件。预览仅检查结构：它验证配置以及运行时实际渲染的 Handlebars 字段（`command`、`args`、
`env` 值、`cwd`、`url` 和 `disabled`），返回脱敏的变更计划和活动实例影响，但不会渲染上下文、连接后端或创建实例。如果任一来源已占用目标名称，创建和重命名都会失败；应用操作必须确认当前预览。

创建定义只会在重载后将其加入模板索引。实例仍由后续匹配请求按需创建。成功重载后，重命名或结构化/运行时渲染字段变更会使现有活动实例退出；之后的匹配请求会使用更新后的定义创建新实例。仅修改指令、禁用工具或工具描述等元数据时会保留现有实例。

完整管理员还可以禁用或重新启用一个来源限定的模板定义。只读预览会绑定当前定义指纹、目标状态、重载计划和当前活动实例数量。带上下文渲染的 `disabled` 表达式在操作层视为已启用；执行禁用时会明确地把该表达式替换为字面量 `true`，不会求值或暴露请求上下文。重新启用会移除 `disabled`。同名静态定义始终不会被修改。

禁用写入持久配置后，只有重载成功才会执行实例退出。写入前处于活动状态的所有实例都会退出，其持久和临时请求会话成员关系都会移除，但请求会话本身会保留。结果会分别报告持久化、重载和已观测的退出状态，因此配置已写入但重载失败时不会声称运行时已成功。重新启用只恢复模板索引资格，不会重建旧实例或成员关系；之后只有新的匹配请求才会按需创建实例。重复执行已满足的状态变更会返回未变化，并跳过备份、写入、重载和退出。

删除操作先对唯一的来源限定定义执行只读结构预览。预览会显示脱敏后的定义、权威或被遮蔽状态、同名另一来源的保留情况、必须创建的恢复副本、预期重载以及运行时影响。应用时必须输入服务器提供的完整可打印标识，例如 `mcpTemplates/workspace-worker`；不接受裸名称。如果定义、来源或预览发生变化，删除会在写入前停止。成功删除模板定义后，运行时作用域会重载，并报告写入前处于活动状态的实例退出结果。删除不会级联到预设、过滤器、凭据、注册表状态、另一来源或单个实例配置。
