...
MCP 服务器模板支持动态、上下文感知的服务器配置。您无需硬编码服务器设置，而是可以由服务器拥有的集成定义模板配置，并根据受信任的运行时上下文（如当前项目、用户、环境或连接）自动调整。
...
::: warning 公共客户端上下文不受信任

通过 HTTP、SSE、streamable HTTP MCP 或 REST 入口传入的 `_meta.context`、`context` 查询参数及客户端元数据均不受信任。它们不会渲染 `mcpTemplates`，也不能影响模板进程的 `command`、`args`、`cwd` 或 `env`。模板服务器只能由服务器进程内创建并明确标记为受信任的服务器拥有上下文启动。

`mcp-session-id` 可以路由到已有会话中已作用域化的连接。应将它作为需要保护的 bearer 路由能力，而不是用户身份、认证或上下文受信任的证明；它不能触发模板渲染。公共 HTTP、SSE、streamable HTTP、proxy、`run` 与 `inspect` 路径只能使用静态目录及已创建的会话作用域连接，在具备服务器可验证的客户端上下文能力之前，不能按客户端上下文创建模板。
...
| ---------- | ------------------------- | --------------------------- |
| 配置       | 启动时固定值              | 基于上下文的动态值          |
| 上下文感知 | 无                        | 受信任的服务器拥有上下文    |
| 多实例     | 每个配置一个实例          | 每个受信任上下文多个实例    |
| 生命周期   | 始终运行                  | 由受信任集成按需创建        |
...
项目级别的上下文增强允许服务器拥有的集成将自定义元数据注入模板。在项目根目录中创建 `.1mcprc` 文件：

该文件不会让公共客户端、proxy 或 HTTP 请求数据变为受信任数据。公共请求不能使用 `.1mcprc` 数据触发模板渲染。
...
2. 确认服务器拥有的集成正在创建受信任上下文；公共 HTTP、SSE、streamable HTTP、proxy 与 REST 请求会有意不渲染模板
3. 验证变量名称与上下文结构匹配
