...
- <span v-pre>`{{context.sessionId}}`</span> - 连接会话
...
| 变量                | 类型   | 描述            | 示例                   |
| ------------------- | ------ | --------------- | ---------------------- |
| `context.path`      | string | 工作目录路径    | `/Users/dev/myapp`     |
| `context.timestamp` | string | ISO 8601 时间戳 | `2025-01-25T10:30:00Z` |
| `context.sessionId` | string | 唯一连接会话 ID | `sess_abc123`          |
| `context.version`   | string | 1MCP 版本       | `0.29.0`               |
...
| 设置               | 类型                   | 默认值     | 描述                       |
| ------------------ | ---------------------- | ---------- | -------------------------- |
| `validateOnReload` | boolean                | `false`    | 重新加载配置时验证模板     |
