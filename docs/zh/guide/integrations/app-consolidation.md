...
整合会将备份集中存储在 1MCP 配置目录下的 `<config-dir>/backups/apps/<app-name>/`。使用默认配置目录时，macOS 和 Linux 为 `~/.config/1mcp/backups/apps/<app-name>/`，Windows 为 `%APPDATA%\\1mcp\\backups\\apps\\<app-name>\\`。已有的旧备份仍可被发现，但新的整合备份使用集中位置。
