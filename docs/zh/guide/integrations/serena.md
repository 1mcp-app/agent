...
::: warning 模板配置需要服务器拥有的受信任上下文

下方的静态配置是公共 HTTP、SSE、streamable HTTP、REST 与 proxy 的受支持配置。本页的模板示例只适用于服务器拥有的集成创建了明确受信任的进程内上下文时。公共 `_meta.context`、`context` 查询参数、客户端元数据和 `.1mcprc` 的 `context` 字段不能渲染 `mcpTemplates`，也不能影响 Serena 的命令、项目路径或环境。`mcp-session-id` 只能路由已有的会话作用域连接；它不是用户身份或受信任上下文。
...
在服务器拥有的受信任集成中，将 Serena 与 1MCP 的模板系统结合可以实现：

- **自动项目检测**: 模板通过 <span v-pre>`{{project.path}}`</span> 动态注入项目路径
- **上下文感知配置**: 根据已验证的客户端类型（IDE vs CLI）提供不同的工具集
- **基于环境的控制**: 在开发环境中启用语义分析，在生产环境中禁用
- **零手动配置**: 受信任的服务器拥有项目上下文会自动流向 Serena
...
### 基于模板的配置（仅限服务器拥有的受信任集成）

仅在服务器拥有的受信任集成中使用模板实现自动项目检测：
...
**工作原理**: 当服务器拥有的集成创建受信任上下文时，1MCP 可以：
...
Serena 需要项目根目录进行分析。服务器拥有的受信任集成可以使用 <span v-pre>`{{project.path}}`</span> 自动注入：
...
**重要**: 在服务器拥有的受信任集成中，当渲染的模板配置相同时，1MCP 可以共享同一个 Serena 实例。这意味着：

- 附着到同一受信任集成、在**同一台机器上**使用**相同上下文**处理**同一项目**的会话可以共享一个 Serena 实例
- 每个唯一的项目路径获得自己的专用 Serena 实例
- 不同的上下文（例如 `claude-code` vs `ide`）获得独立的实例

**示例**: 服务器拥有的集成可以将一个受信任的 `claude-code` 模板实例作用域化给使用相同项目上下文的会话，而单独的受信任 `ide` 上下文会获得另一个实例。公共 CLI、proxy 与 HTTP 客户端元数据不能通过渲染模板创建或选择这些实例。
...
Serena 的 `--context` 参数根据客户端类型控制可用的工具。在服务器拥有的受信任集成中，使用模板条件选择适当的上下文：
...
在服务器拥有的受信任集成中，在 `.1mcprc` 中定义自定义元数据并在模板中引用。公共请求不能使用此文件触发渲染：
...
**解决方案**: 先确认服务器拥有的集成正在创建受信任上下文。公共 HTTP、SSE、streamable HTTP、REST 与 proxy 请求会有意不渲染 Serena 模板。在受信任集成中，确保使用 `mcpTemplates` 而不是 `mcpServers`：
...
1. 通过启用调试日志验证 <span v-pre>`{{project.path}}`</span> 正确解析
2. 初始化项目：在项目根目录运行 `serena project create`
