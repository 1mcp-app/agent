...
::: warning 目前请使用静态配置
本页的模板示例描述的是预期工作流，但公开 HTTP、REST 和 `1mcp proxy` 上下文目前不能创建示例中的 Serena 模板实例。请使用下面固定路径的静态配置；安全约定见 [MCP 服务器模板](/zh/guide/mcp-server-templates#公网上下文不能创建模板服务器)。
:::
...
Serena 需要项目根目录进行分析。未来的可信上下文工作流可以使用 <span v-pre>`{{project.path}}`</span> 注入该路径：
...
**预期行为**：可信渲染可用后，渲染出的模板配置相同时，1MCP 可以共享同一个 Serena 实例。其设计含义是：
...
Serena 的 `--context` 参数根据客户端类型控制可用的工具。未来经过认证的服务端集成可以使用模板条件选择适当的上下文：
...
未来经过认证的服务端上下文集成可以根据连接的客户端选择 Serena 上下文：
...
对于未来经过认证的集成，`.1mcprc` 可以定义供模板使用的自定义元数据：
...
**重要**：在预期的模板设计中，1MCP 模板将配置 Serena 服务器实例（CLI 参数），而 Serena 的配置文件控制分析行为（索引首选项、语言设置）。

## 完整示例

### 示例 1：多环境设置

未来示例：在开发环境中启用语义分析，在生产环境中禁用：
...
**症状**：公开 HTTP、REST 或 `1mcp proxy` 客户端没有创建 Serena 模板，或者模板字段仍为字面量。

**解决方案**：这是预期的默认拒绝行为。请使用[基本静态配置](#基本静态配置)，在 `mcpServers` 中填写固定的绝对项目路径。在此版本中，把定义移入 `mcpTemplates` 或调试 <span v-pre>`{{project.path}}`</span> 都不能启用公开模板渲染。
...
1. 验证静态 `mcpServers` 配置中的固定绝对项目路径
