---
title: Serve 命令
description: 启动 1MCP 主运行时，并将其用于 CLI 模式和原生 HTTP MCP 客户端。
---
...
它负责聚合你配置好的静态 MCP 服务器、暴露 HTTP MCP 入口，并初始化预设与指令聚合。
...
- 静态服务器从启动配置创建。
- 公开 HTTP 与 proxy 上下文不会创建模板服务器；参见 [MCP 服务器模板](/zh/guide/mcp-server-templates#公网上下文不能创建模板服务器)。
