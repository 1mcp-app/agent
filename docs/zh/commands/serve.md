...
- **已有记录：** 缺少进程身份或新锁标记的版本 1 元数据，无法仅凭数字 PID 是否存活来安全回收。升级前请先正常停止旧运行时。如果崩溃后仍有旧元数据，请先确认使用该作用域的所有运行时、supervisor、worker 和生命周期命令都已停止，然后删除遗留的 `runtime.owner`、`runtime.stop`、`server.pid`；后台运行时还需删除 `background-runtime.json`。不要在启动时自动执行无差别清理。
