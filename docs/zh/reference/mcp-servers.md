...
MCP 服务器模板支持动态、上下文感知的服务器配置。服务器拥有的集成可以定义模板配置，并根据受信任的运行时上下文（如当前项目、用户、环境或连接）自动调整，而无需硬编码服务器设置。

### 模板配置

模板在配置的 `mcpTemplates` 部分中定义：

> 安全边界：公共 HTTP、SSE、streamable HTTP 与 REST 请求上下文不能渲染模板或影响进程设置。模板渲染需要服务器拥有、在进程内受信任的上下文。会话 ID 只能路由已经创建的会话作用域连接；它不是用户身份或受信任上下文。请参阅 [MCP 服务器模板指南](/zh/guide/mcp-server-templates) 了解当前 HTTP 兼容性限制。
...
服务器拥有的集成创建受信任模板上下文时，可通过 `.1mcprc` 文件增强项目级别的上下文。公共请求或 proxy 不能用此文件使客户端上下文变为受信任上下文，也不能启动模板渲染：
