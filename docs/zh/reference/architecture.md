...
1. 仅支持 stdio 的客户端启动 `1mcp proxy`。
2. `proxy` 发现一个运行中的 `serve` 实例，并把 stdio 流量转发到 HTTP 运行时。
3. 过程中仍可应用预设或过滤，但 `proxy` 的定位始终是兼容桥，而不是主要产品入口。
...
在需要时，模板服务器解析是带有会话感知能力的。inspect 路由与 tool 路由可以基于请求上下文和 session ID 初始化模板服务器，使客户端看到正确的上下文化服务目录。
...
适用于能直接连接聚合运行时的原生 MCP 客户端。

### `proxy`

`1mcp proxy` 用于 stdio 兼容。它把本地 stdio 客户端桥接到运行中的 HTTP 运行时，定位应当是兼容层，而不是默认的架构中心。

## 安全与运维边界

- `serve` 是运行时边界，认证、限流、请求处理与健康检查都在这里。
- 模板解析发生在运行时内部，并受提供给它的客户端或会话上下文约束。
- `proxy` 不会给 stdio-only 客户端凭空增加 OAuth 能力；如果客户端本身不能完成认证，这个限制仍然存在。
