...
2. 初始化预设管理，包括预设变更通知。
3. 根据启动配置创建静态服务器传输。
4. 模板定义会被加载，但此版本中的公开客户端或会话上下文不能将其物化为服务器实例。
...
3. 过程中仍可应用预设、过滤器和标签，因此 `proxy` 是 CLI 模式之后推荐的兼容性回退路径。
...
模板服务器管理器负责上下文渲染、实例共享、路由和清理。此版本中没有经过认证的服务端上下文集成调用该渲染路径。
...
公开的 inspect、tool、直接 HTTP 和 proxy 路由不会根据调用方上下文初始化模板服务器。完整的可用性和信任边界约定见 [MCP 服务器模板](/zh/guide/mcp-server-templates#公网上下文不能创建模板服务器)。
...
适用于能通过 streamable HTTP 直接连接聚合运行时的原生 MCP 客户端。

### `proxy`

`1mcp proxy` 是 CLI 模式之后的最大兼容性入口。它把本地 stdio 客户端桥接到运行中的 HTTP 运行时，并可应用预设、过滤器和标签选择。

## 安全与运维边界

- `serve` 是运行时边界，认证、限流、请求处理与健康检查都在这里。
- 调用方提供的 HTTP、REST、proxy 和持久化会话上下文不具备模板渲染权限。
