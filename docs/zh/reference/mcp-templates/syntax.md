...
公开客户端上下文目前不能调用此渲染语法；参见 [MCP 服务器模板安全约定](/zh/guide/mcp-server-templates#公网上下文不能创建模板服务器)。
...
内部渲染器支持以下五步工作流，但此版本没有调用它的、经过认证的服务端上下文集成：

### 步骤 1：上下文收集

未来经过授权的集成将提供服务端拥有的上下文，例如：
...
渲染器将从 `mcp.json` 的 `mcpTemplates` 中查找符合过滤条件（标签、预设）的模板。
...
只有在可信渲染和验证完成后，才会创建服务器实例。

### 缓存

可信渲染可用后，`cacheContext` 会按上下文哈希缓存结果，以避免重新处理相同的上下文。
