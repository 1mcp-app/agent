...
公共 HTTP、SSE、streamable HTTP 与 REST 请求上下文仅作为元数据。`_meta.context`、`context` 查询参数和客户端元数据中的值永远不会进入模板渲染或进程创建，也不能影响 `command`、`args`、`cwd` 或 `env`。以下流程仅适用于已在服务器进程内明确标记为受信任的服务器拥有上下文。会话 ID 只能路由已有的会话作用域连接；它不是用户身份或受信任上下文，不能启动此流程。

:::

### 步骤 1：上下文收集

当服务器拥有的集成创建受信任模板上下文时，1MCP 可以从以下位置收集上下文：
...
1MCP 从 `mcp.json` 的 `mcpTemplates` 中查找与受信任集成的过滤条件（标签、预设）匹配的模板。
...
使用渲染后的配置创建服务器实例，并将其作用域化到受信任集成的会话。公共路由只能访问已创建的作用域化实例，不能根据请求数据创建实例。
