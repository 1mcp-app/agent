...
- **Other platforms:** macOS uses `ps` process start time, with one-second precision; Windows uses PowerShell process start ticks. Missing tools, denied access, unsupported platforms, or mismatched execution contexts remain uncertain and fail closed. Linux file-lock recovery does not apply on these platforms.
