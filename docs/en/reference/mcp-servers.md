...
- `stderr` (string or integer, optional): Controls the child process stderr target. Omitted or `"pipe"` captures stderr in the 1MCP logger with bounded output (8 KiB per line, 20 unique lines per 10 seconds, and repeated-line summaries every 5 seconds). Use `"inherit"` for direct terminal output, `"ignore"` to discard it, or a non-negative file descriptor to pass it through to the child process.
