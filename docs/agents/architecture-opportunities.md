...
- Milestone 6 OAuth flow is implemented: `oauthAuthorizationFlow.ts` owns consent submission, localhost CLI-token creation, backend OAuth dashboard fact shaping, backend OAuth start/restart, and backend OAuth callback completion/loading-ready notification. HTTP routes now delegate those state mutations and dashboard facts through structured flow operations while retaining dashboard HTML rendering and transport response mapping. The first Instructions Distribution slice is implemented: `instructionsDistribution.ts` owns eager-inspection policy, instruction detail assembly, managed startup-doc content, startup-reference rendering, and legacy managed-reference cleanup. `instructions` and `cli-setup` now delegate those policies while retaining inspect calls, output formatting, hooks, path resolution, JSON merging, and file writes.

## Next Execution Target

Continue Milestone 6 by broadening **Instructions Distribution** around any remaining startup-doc/client-surface policy that still lives in adapters, then decide whether `instructions` should use the shared **Client Surface Attachment** model from Milestone 5. The OAuth Authorization Flow slices now live behind `src/auth/oauthAuthorizationFlow.ts`.
