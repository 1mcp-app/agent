...
- Milestone 6 OAuth flow is implemented: `oauthAuthorizationFlow.ts` owns consent submission, localhost CLI-token creation, backend OAuth dashboard fact shaping, backend OAuth start/restart, and backend OAuth callback completion/loading-ready notification. HTTP routes now delegate those state mutations and dashboard facts through structured flow operations while retaining dashboard HTML rendering and transport response mapping. The first Instructions Distribution slice is implemented: `instructionsDistribution.ts` owns eager-inspection policy, instruction detail collection/assembly, managed startup-doc content, startup-reference rendering, and legacy managed-reference cleanup. `instructions` now uses its own reusable **Client Surface Attachment** identity while delegating distribution policy; `cli-setup` delegates managed-doc policy while retaining hooks, path resolution, JSON merging, and file writes.

## Next Execution Target

Continue Milestone 6 by broadening adapter/regression coverage for `instructions` as its own **Client Surface** if needed, then review whether the remaining No.10 checklist items are fully covered. The OAuth Authorization Flow slices now live behind `src/auth/oauthAuthorizationFlow.ts`.
