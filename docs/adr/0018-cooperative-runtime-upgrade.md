...
The behavioral contract is accepted and implemented for compatible background runtimes. The implementation and verification boundaries are recorded in [cooperative runtime upgrade design](../agents/cooperative-runtime-upgrade-design.md). Normal cooperative replacement must preserve ownership and context-authority protections; timeout or a public identity response alone never authorizes takeover. Existing recovery guarantees remain authoritative.
